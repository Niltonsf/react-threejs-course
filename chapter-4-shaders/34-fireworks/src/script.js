...
window.addEventListener('resize', () =>
{
    ...
    sizes.pixelRatio.set(Math.min(window.devicePixelRatio, 2))
...
const createFirwork = (count, position, size, texture, radius, color) => {
    // Geometry    
    ...
    for(let i = 0; i < count; i++)
    {
        ...
        positionsArray[i3] = position.x - 0.5
        positionsArray[i3 + 1] = position.y - 0.5
        positionsArray[i3 + 2] = position.z - 0.5
...
const createRandomFirework = () => {
    ...
    createFirwork(count, position, size, texture, radius, color)   
}
